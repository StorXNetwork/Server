...
module.exports = (Model, App) => {
  ...
  const FindUserByUuid = (userUuid) =>
    Model.users.findOne({ where: { uuid: { [Op.eq]: userUuid } } });

  const FindUserObjByEmail = (email) =>
    Model.users.findOne({ where: { email: { [Op.eq]: email } } });

  const GetUserCredit = (userUuid) =>
    Model.users
  ...
  const ResendActivationEmail = (user) =>
    axios.post(`${process.env.STORJ_BRIDGE}/activations`, { email: user });
  ...
  return {
    ...
    ActivateUser,
    GetUserBucket,
    GenerateTestApplicationKey,
    GenerateLiveApplicationKey,
  };
};
